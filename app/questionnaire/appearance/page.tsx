...
const STEPS = [
  {
    ...
    subtitle: '你更喜欢哪种触感',
    options: [
      { value: 1, label: 'FLUFFY', meta: '毛茸茸，长毛或蓬松' },
      { value: 2, label: 'SMOOTH', meta: '光滑短毛，易打理' },
      { value: 3, label: 'SPECIAL', meta: '无毛、鳞片、羽毛等独特触感' },
      { value: 0, label: 'NO PREFERENCE', meta: '无所谓，都可以' },
    ],
  },
  {
    id: 'colorPreference',
    ...
    subtitle: '你偏好的外观色调',
    options: [
      { value: 1, label: 'WARM', meta: '暖色系（橘、棕、金）' },
      { value: 2, label: 'COOL', meta: '冷色系（灰、蓝、银）' },
      { value: 3, label: 'CLASSIC', meta: '经典黑白' },
      { value: 4, label: 'COLORFUL', meta: '多彩缤纷' },
      { value: 0, label: 'NO PREFERENCE', meta: '无所谓' },
    ],
  },
  {
    id: 'sizePreference',
    ...
    subtitle: '你期望的宠物体型',
    options: [
      { value: 1, label: 'TINY', meta: '口袋大小，掌心萌物' },
      { value: 2, label: 'SMALL', meta: '小巧灵活，轻松抱起' },
      { value: 3, label: 'MEDIUM', meta: '中等体型，拥抱刚好' },
      { value: 4, label: 'LARGE', meta: '大型伙伴，安全感满满' },
      { value: 0, label: 'NO PREFERENCE', meta: '无所谓' },
    ],
  },
];
    ...
    <div className="relative h-screen w-full bg-[#F6F5F2] text-[#1A1A1A] overflow-hidden selection:bg-[#7A2E2E]/10">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-[#F6F5F2]/80 backdrop-blur-md border-b border-black/5">
        <div className="flex items-center justify-between px-6 md:px-12 py-6">
          ...
          <span className="font-inter text-[10px] tracking-[0.3em] text-[#1A1A1A]/30 font-light">05 / 07</span>
        </div>
      </nav>
          ...
          {STEPS.map((s, i) => (
            ...
            >
              <span className={`font-inter text-[9px] tracking-[0.2em] uppercase transition-opacity duration-700 ${activeStep === i ? 'opacity-40' : 'opacity-0'}`}>
                {s.id.replace('Preference', '').toUpperCase()}
              </span>
              <div className={`h-[1px] transition-all duration-1000 ${activeStep === i ? 'w-16 bg-[#7A2E2E]' : 'w-8 bg-black/10'}`} />
            </button>
          ))}
        </div>
        <div className="font-inter-label text-[10px] opacity-20">Step 05</div>
      </footer>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <h3 className={`font-inter text-base md:text-lg lg:text-xl font-medium tracking-[0.15em] md:tracking-[0.2em] transition-colors duration-300
                            ${isSelected ? 'text-[#7A2E2E]' : 'group-hover:text-[#7A2E2E]'}`}>
                            {opt.label}
                          </h3>
                          <p className="font-inter text-[8px] md:text-[9px] tracking-[0.2em] opacity-30 uppercase mt-1.5 font-light">
                            {opt.meta}
                          </p>
                        </div>
                        <motion.span
